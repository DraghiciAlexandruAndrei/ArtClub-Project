...
{
    [Authorize] // Restricționăm tot controller-ul la utilizatori logați
    public class AccountController : Controller
    {
        ...
        [AllowAnonymous] // Permitem vizualizarea listei de utilizatori pentru toți (sau poți pune [Authorize(Roles="Admin")])
        ...
        public async Task<IActionResult> ProcessUpgrade()
        {
            ...
            if (success)
            {
                ...
                if (oldRoles.Any()) await _userManager.RemoveFromRolesAsync(user, oldRoles);
        ...
        public async Task<IActionResult> BecomeMember()
        {
            ...
            TempData["StatusMessage"] = "Plată confirmată! Acum ești membru activ ArtClub.";
            return RedirectToAction(nameof(MyProfile));
        }
        ...
        {
            bool result = accept ? await _invitationService.AcceptInvitationAsync(id) : await _invitationService.DeclineInvitationAsync(id);

            if (result) TempData["StatusMessage"] = accept ? "Ai acceptat invitația!" : "Ai refuzat invitația.";
            else TempData["ErrorMessage"] = "A apărut o eroare la procesarea invitației.";

            return RedirectToAction(nameof(MyProfile));
        }

        [HttpPost]
        [Authorize(Roles = "Admin")] // Doar Adminii pot șterge utilizatori
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> DeleteConfirmed(int id)
        {
            ...
            {
                TempData["ErrorMessage"] = "Nu îți poți șterge propriul cont.";
                return RedirectToAction(nameof(Index));
            }
            ...
            if (result.Succeeded)
            {
                TempData["StatusMessage"] = $"Contul lui {user.UserName} a fost eliminat.";
                return RedirectToAction(nameof(Index));
            }

            TempData["ErrorMessage"] = "Eroare la ștergere.";
            return RedirectToAction(nameof(Index));
        }
        ...
        public async Task<IActionResult> Dashboard()
        {
            ...
            var model = new MemberDashboardViewModel
            {
                ...
                RecentInvitations = invitations.Where(i => i.Status == InvitationStatus.Pending).Take(3).Select(i => new InvitationInboxViewModel
                {
                    InvitationId = i.Id,
                ...
                UpcomingEvents = organized.Where(e => e.Reservation?.StartTime > DateTime.Now).Select(e => new EventSummaryViewModel
                {
                    Title = e.Title,
                    StartDate = e.Reservation?.StartTime ?? DateTime.Now,
